...
namespace Esoft_Project
{
    public partial class FormDeal : Form
    {
        ...
        void ShowSupply()
        {
            ...
            {
                string[] item = { supplySet.Id.ToString() + ". ", "Риелтор: " + supplySet.agent.LastName, "Клиент: " + supplySet.ClientsSet.LastName };
        ...
        void ShowDemand()
        {
            ...
            {
                string[] item = { demandSet.Id.ToString() + ". ", "Риэлтор: " + demandSet.agent.LastName, "Клиент: " + demandSet.ClientsSet.LastName };
        ...
        void Deductions()
        {
            if (comboBoxSupply.SelectedItem != null && comboBoxDemand.SelectedItem != null)
            {
                ...
                if (demandSet.agent.DealShare != null)
                {
                    double agentCustomerDeductions = customerCompanyDeductions * Convert.ToDouble(demandSet.agent.DealShare) / 100.00;
            ...
            if (comboBoxSupply.SelectedItem != null)
            {
                ...
                if (supplySet.agent.DealShare != null)
                {
                    double agentSellerDeductions = sellerCompanyDeductions * Convert.ToDouble(supplySet.agent.DealShare) / 100.00;
        ...
        void ShowDealSet()
        {
            ...
            foreach (DealSet deal in Program.wftDb.DealSet)
            {
                ListViewItem item = new ListViewItem(new string[]
                {
                    ...
                    deal.SupplySet.agent.LastName,
                    deal.DemandSet.ClientsSet.LastName,
                    deal.DemandSet.agent.LastName,
                    "г. "+ deal.SupplySet.RealEstateSet.Address_City+", ул. "+deal.SupplySet.RealEstateSet.Address_Street+", д. "+deal.SupplySet.RealEstateSet.Address_House+", кв. "+deal.SupplySet.RealEstateSet.Address_Number, deal.SupplySet.Price.ToString()
                });
                item.Tag = deal;
        ...
        private void buttonEdit_Click(object sender, EventArgs e)
        {
            if (listViewDealSet.SelectedItems.Count == 1)
            {
                ...
                deal.IdSupply = Convert.ToInt32(comboBoxSupply.SelectedItem.ToString().Split('.')[0]);
                deal.IdDemand = Convert.ToInt32(comboBoxSupply.SelectedItem.ToString().Split('.')[0]);
                Program.wftDb.SaveChanges();
                ShowDealSet();
            }
        }
